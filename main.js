...
var SITE_DATA = {
  ...
  stats: [
    { label: 'Delegates',             value: 50,   suffix: '+' },
    { label: 'Conferences Attended',  value: 25,   suffix: '+' },
    { label: 'Awards Won',            value: 100,  suffix: '+' },
  ...
  /* ---- Conferences page: UPCOMING ------------------------------------
     ...
     IndeMUN, FHSMUN and DavisMUN read TBD: the dates on the sheet for those
     three were LAST season's, and this season's are not set yet. Fill them in
     when they are announced and move the rows into date order.
     BearMUN's year and NHSMUN's spring-break date were cut off in the source
     screenshot and are still unconfirmed.
     ...
     Committee assignments are deliberately not shown — they change too often
     to keep accurate on a static page. Add a `committees: [...]` field back and
     restore the block in renderUpcoming() if that changes. */
  upcomingConferences: [
    { name: 'FREMUNC',      date: 'September 26',           location: 'Irvington High School', status: 'tbd' },
    { name: 'GunnMUN',      date: 'October 24, 2026',       location: 'Gunn High School', status: 'tbd' },
    { name: 'SMUNC',        date: 'November 6–8, 2026',     location: 'Stanford University', status: 'tbd' },
    { name: 'BearMUN',      date: 'December 5–6, 2027',     location: 'UC Berkeley', status: 'tbd' },
    { name: 'NorCal MUN',   date: 'January 22–23, 2027',    location: 'Foothill High School', status: 'tbd' },
    { name: 'Harvard MUN',  date: 'January 28–31, 2027',    location: 'Harvard University', status: 'tbd' },
    { name: 'SCVMUN',       date: 'January 29–30, 2027',    location: 'Santa Teresa High School', status: 'tbd' },
    { name: 'SFMUN',        date: 'February 21 (approx.)',  location: 'Lowell High School', status: 'tbd' },
    { name: 'IndeMUN',      date: 'TBD',                    location: 'Independence High School', status: 'tbd' },
    { name: 'BMUN',         date: 'March 5–7, 2027',        location: 'UC Berkeley', status: 'tbd' },
    { name: 'NHSMUN',       date: 'Spring break, 2027',     location: 'New York City', status: 'tbd' },
    { name: 'FHSMUN',       date: 'TBD',                    location: 'TBD', status: 'tbd' },
    { name: 'DavisMUN',     date: 'TBD',                    location: 'UC Davis', status: 'tbd' },
    { name: 'MiniMUN',      date: 'TBD',                    location: 'Irvington High School', status: 'tbd' }
  ],
  ...
  /* ---- Conferences page: PAST ----------------------------------------
     ...
     awards: [] renders no awards line at all — the cards deliberately do not
     list awards. Put strings in the array and the list reappears.
  ...
  pastConferences: [
    {
      ...
      link: 'https://fremunc26.vercel.app/',
      linkLabel: 'Visit the FREMUNC site'
    },
    { name: 'Stanford MUN', season: '2025–26', awards: [], photo: null, photoAlt: '' },
    {
      name: 'BearMUN',
      season: '2025–26',
    ...
    },
    {
      ...
      season: '2025–26',
      awards: [],
      photo: 'assets/scvmun-2026.jpg',
      photoAlt: 'Two Irvington delegates in the Santa Teresa High School gym holding up Outstanding Delegate certificates from Santa Clara Valley Model United Nations, both representing the Philippines in the UN Environment Programme.',
      photoW: 1600, photoH: 1200
    },
    {
      name: 'IndeMUN',
      season: '2025–26',
      awards: [],
      photo: 'assets/indemun-2026.jpg',
      photoAlt: 'The Irvington delegation of twelve gathered outside the administrative services building at IndeMUN, several holding award certificates and two holding Best Delegate gavels.',
      photoW: 1600, photoH: 1200
    },
    { name: 'Titan MUNC', season: '2025–26', awards: [], photo: null, photoAlt: '' },
    {
      name: 'BMUN',
      season: '2025–26',
      awards: [],
      photo: 'assets/bmun-2026.jpg',
      photoAlt: 'The Irvington delegation of fourteen on a sunlit lawn beneath a large cedar at BMUN, with other delegates gathered in the background.',
      photoW: 1600, photoH: 1200
    },
    { name: 'NHSMUN',     season: '2025–26', awards: [], photo: null, photoAlt: '' }
  ],

  /* ---- Home page: DIAMOND GALLERY -------------------------------------
     ██ TUNE CROPS HERE ██
     The lattice is a 6x6 grid rotated 45°, so each tile is a diamond showing
     the CENTRAL SQUARE of its photo. That is geometric — a diamond cannot show
     a wide group shot whole, and edges of the frame are always lost.

     `focus` is object-position. Only one of its two numbers does anything, and
     which one depends on the source: a LANDSCAPE photo in a square tile
     overflows horizontally, so only the X value moves it; a PORTRAIT photo
     overflows vertically, so only Y moves it. All of these are landscape
     except throwback-03. For centred group shots 50% is already correct.

     If a photo still crops badly, add `zoom` (default 1.415, the minimum that
     covers) and `shiftY` to that entry — see the note in styles.css §16b.

     col / row are 1-indexed on the 6x6 grid; span: 2 makes the centre hero.
     Empty cells are intentional. Below 700px placement is ignored entirely. */
  gallery: [
    /* --- the centre hero diamond --- */
    { src: 'assets/delegation-awards.jpg', col: 3, row: 3, span: 2, focus: '50% 50%',
      alt: 'The full Irvington delegation gathered outside the venue after a conference, many holding award certificates and gavels.' },

    /* --- inner ring --- */
    { src: 'assets/team-bearmun.jpg?v=2',      col: 2, row: 2, focus: '50% 50%',
      alt: 'The delegation on the steps of Sproul Hall at UC Berkeley with the Best Large Delegation plaque from BearMUN 2025.' },
    { src: 'assets/throwback-01.jpg',          col: 3, row: 2, focus: '50% 50%',
      alt: 'The FREMUNC secretariat, chairs and staff assembled on stage.' },
    { src: 'assets/throwback-02.jpg',          col: 4, row: 2, focus: '50% 50%',
      alt: 'Irvington delegates filling a row of auditorium seats during a conference session.' },
    { src: 'assets/fremunc-2025.jpg',          col: 5, row: 2, focus: '50% 50%',
      alt: 'The FREMUNC secretariat on stage holding bouquets at the close of the conference.' },

    /* the one portrait source in the set, so Y is the live knob here */
    { src: 'assets/throwback-03.jpg',          col: 2, row: 3, focus: '50% 32%',
      alt: 'Two delegates holding Outstanding Delegate certificates in a school gymnasium.' },
    { src: 'assets/throwback-04.jpg',          col: 5, row: 3, focus: '50% 50%',
      alt: 'Delegates on a stone staircase holding their award certificates.' },

    { src: 'assets/throwback-05.jpg',          col: 2, row: 4, focus: '50% 50%',
      alt: 'A line of delegates in formal dress holding certificates outside the conference venue.' },
    { src: 'assets/throwback-06.jpg',          col: 5, row: 4, focus: '50% 50%',
      alt: 'Two delegates with a Best Delegate certificate on the steps at night.' },

    { src: 'assets/throwback-07.jpg',          col: 2, row: 5, focus: '50% 50%',
      alt: 'Delegates gathered around a long table at a team dinner during a conference weekend.' },
    { src: 'assets/throwback-08.jpg',          col: 3, row: 5, focus: '50% 50%',
      alt: 'Delegates holding certificates in front of a painted school mural.' },
    { src: 'assets/throwback-09.jpg',          col: 4, row: 5, focus: '50% 50%',
      alt: 'Delegates posing together indoors at the end of a conference day.' },
    { src: 'assets/throwback-10.jpg',          col: 5, row: 5, focus: '50% 50%',
      alt: 'A keynote speaker addressing delegates from the stage at NHSMUN.' },

    /* --- outer protrusions: these break the silhouette on purpose --- */
    { src: 'assets/throwback-11.jpg',          col: 3, row: 1, focus: '50% 50%',
      alt: 'Delegates seated together in a darkened conference hall during opening ceremonies.' },
    { src: 'assets/delegates-in-session.jpg',  col: 6, row: 3, focus: '50% 50%',
      alt: 'Irvington delegates seated in an auditorium wearing conference badges.' },
    { src: 'assets/delegation-campus.jpg',     col: 4, row: 6, focus: '50% 50%',
      alt: 'Delegates on a shaded campus lawn between committee sessions.' }
  ],
  ...
  officers: [
    /* --- Executive Board --- */
    { group: 'exec', name: 'Sonia Puri',      role: 'President',      grade: '', bio: '', photo: null },
    { group: 'exec', name: 'Shriya Surana',   role: 'President',      grade: '', bio: '', photo: null },
    { group: 'exec', name: 'Utkarsh Sinha',   role: 'Vice President', grade: '', bio: '', photo: null },
    { group: 'exec', name: 'Nitya Misra',     role: 'Vice President', grade: '', bio: '', photo: null },
    ...
    { group: 'captains', name: 'Anantiga Ramesh', role: 'Captain, Travel Team',      grade: '', bio: '', photo: null },
    { group: 'captains', name: 'Shaan Sood',      role: 'Captain, General Assembly', grade: '', bio: '', photo: null },
    { group: 'captains', name: 'Naisha Vora',     role: 'Captain, General Assembly', grade: '', bio: '', photo: null },
    { group: 'captains', name: 'Utkarsh Sinha',   role: 'Captain, Crisis',           grade: '', bio: '', photo: null },

    /* --- Conference Leadership --- */
    { group: 'conference', name: 'Matthew Miu',   role: 'Director General, FREMUNC & MiniMUN', grade: '', bio: '', photo: null },
    { group: 'conference', name: 'Yumna Zainab',  role: 'Director General, FREMUNC & MiniMUN', grade: '', bio: '', photo: null },
    { group: 'conference', name: 'Kavish Poddar', role: 'Technical Projects Director',         grade: '', bio: '', photo: null },

    /* --- Junior Officers --- */
    { group: 'junior', name: 'Elina Mangla',      role: 'Junior Officer', grade: '', bio: '', photo: null },
    { group: 'junior', name: 'Sahil Markandeya',  role: 'Junior Officer', grade: '', bio: '', photo: null },
    { group: 'junior', name: 'Pradyun Adabala',   role: 'Junior Officer', grade: '', bio: '', photo: null },

    /* --- Middle School MUN --- */
    { group: 'middle', name: 'Kshitij Markandeya', role: 'Middle School MUN Leader', grade: '', bio: '', photo: null },
    { group: 'middle', name: 'Riya Vyas',          role: 'Middle School MUN Leader', grade: '', bio: '', photo: null }
  ]
...
(function () {
  ...
  // The collage reveals as a unit — the section gets .is-visible and the tiles
  // stagger off that — so it rides the same observer rather than adding a
  // second one.
  function initReveal() {
    var items = document.querySelectorAll('.reveal, .collage');
    if (!items.length) return;
  ...
  function renderPast() {
    ...
    grid.innerHTML = sorted.map(function (c) {
      // Empty array = render no awards list at all. Previously this printed
      // "No awards recorded", which asserts none were won; the cards simply do
      // not cover awards now.
      var awards = (c.awards && c.awards.length)
        ? '<ul class="conf-card__awards">' +
  ...
  /* ==========================================================================
     10. DIAMOND GALLERY + LIGHTBOX
     Tiles are <button>s, so keyboard access and the accessible name come free
     from the image alt — no aria-label duplication.
     ====================================================================== */
  ...
  function initGallery() {
    ...
    stage.innerHTML = photos.map(function (p, i) {
      var style = '--col:' + (p.col || 1) + ';--row:' + (p.row || 1) +
                  ';--span:' + (p.span || 1) + ';--focus:' + (p.focus || '50% 50%') +
                  (p.zoom   ? ';--zoom:' + p.zoom : '') +
                  (p.shiftY ? ';--shiftY:' + p.shiftY : '');
      // The centre hero is the one image worth fetching eagerly; the rest wait.
      var load = p.span === 2 ? 'fetchpriority="high"' : 'loading="lazy"';
      return '<button class="dgal__tile' + (p.span === 2 ? ' dgal__tile--hero' : '') +
               '" type="button" style="' + style + '" data-i="' + i + '">' +
               '<img src="' + esc(p.src) + '" alt="' + esc(p.alt || '') + '" ' +
               load + ' decoding="async">' +
             '</button>';
    }).join('');

    buildLightbox(photos, stage);
  }

  function buildLightbox(photos, stage) {
    var ICON = {
    ...
    function btn(cls, label, path) {
      return '<button class="lightbox__btn ' + cls + '" type="button" aria-label="' + label + '">' +
        '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" ' +
        'stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' + path + '</svg></button>';
    }
    ...
    box.innerHTML =
      btn('lightbox__close', 'Close', ICON.close) +
      btn('lightbox__prev', 'Previous photo', ICON.prev) +
      btn('lightbox__next', 'Next photo', ICON.next) +
    ...
    var index = 0, isOpen = false, lastFocused = null;

    function show(i) {
      index = (i + photos.length) % photos.length;   // wraps both directions
      var p = photos[index];
      img.src = p.src;
      img.alt = p.alt || '';
    ...
    // Takes the triggering tile explicitly rather than reading
    // document.activeElement: a programmatic .click() never focuses the
    // button, so activeElement would be <body> and focus would be dropped on
    // the floor when the viewer closed.
    ...
    function onKey(e) {
      if (e.key === 'Escape')     { e.preventDefault(); close(); }
        ...
        // Only three controls, so the trap is a simple cycle.
        var f = box.querySelectorAll('.lightbox__btn');
    ...
    stage.addEventListener('click', function (e) {
      var t = e.target.closest('.dgal__tile');
      if (t) open(parseInt(t.getAttribute('data-i'), 10) || 0, t);
    });

    box.addEventListener('click', function (e) {
      if (e.target === box) { close(); return; }          // backdrop
      var b = e.target.closest('.lightbox__btn');
      if (!b) return;
      if (b.classList.contains('lightbox__close')) close();
      else if (b.classList.contains('lightbox__prev')) show(index - 1);
      else show(index + 1);
    });
